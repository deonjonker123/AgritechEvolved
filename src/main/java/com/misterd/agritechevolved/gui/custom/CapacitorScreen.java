...
public class CapacitorScreen extends AbstractContainerScreen<CapacitorMenu> {
    ...
    private static final ResourceLocation GUI_TEXTURE = ResourceLocation.fromNamespaceAndPath("agritechevolved", "textures/gui/capacitor_gui.png");
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (maxEnergy > 0) {
            ...
            if (fillWidth > 0) {
                guiGraphics.blit(GUI_TEXTURE, x + BAR_X, y + BAR_Y, 0, 153, fillWidth, BAR_H);
            }
        }
    }
    ...
    protected void renderTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (isHovering(BAR_X, BAR_Y, BAR_W, BAR_H, mouseX, mouseY)) {
            ...
            guiGraphics.renderComponentTooltip(font, List.of(
                    ...
                    Component.translatable("tooltip.agritechevolved.capacitor.energy", fmt.format(energy), fmt.format(maxEnergy)).withStyle(ChatFormatting.WHITE),
                    Component.translatable("tooltip.agritechevolved.capacitor.energy_percentage", String.format("%.1f", pct)).withStyle(ChatFormatting.GRAY),
                    Component.translatable("tooltip.agritechevolved.capacitor.transfer_rate", fmt.format(transferRate)).withStyle(ChatFormatting.AQUA)
            ), mouseX, mouseY);
        }
        if (isHovering(BAR_X, 6, BAR_W, 8, mouseX, mouseY)) {
            ...
            guiGraphics.renderComponentTooltip(font, List.of(
                    ...
                    Component.translatable("tooltip.agritechevolved.capacitor.max_transfer", NumberFormat.getNumberInstance(Locale.US).format(transferRate)).withStyle(ChatFormatting.AQUA)
            ), mouseX, mouseY);
        }
