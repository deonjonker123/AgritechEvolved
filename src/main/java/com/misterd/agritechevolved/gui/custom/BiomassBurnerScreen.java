...
public class BiomassBurnerScreen extends AbstractContainerScreen<BiomassBurnerMenu> {
    ...
    private static final ResourceLocation GUI_TEXTURE = ResourceLocation.fromNamespaceAndPath("agritechevolved", "textures/gui/burner_gui.png");
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (maxProgress > 0) {
            ...
            if (filled > 0) {
                guiGraphics.blit(GUI_TEXTURE, x + PROGRESS_BAR_X, y + PROGRESS_BAR_Y, PROGRESS_BAR_TEX_X, PROGRESS_BAR_TEX_Y, filled, PROGRESS_BAR_H);
            }
        }
        ...
        if (maxEnergy > 0) {
            ...
            if (filled > 0) {
                guiGraphics.blit(GUI_TEXTURE, x + ENERGY_BAR_X, y + ENERGY_BAR_Y + ENERGY_BAR_H - filled, ENERGY_BAR_TEX_X, ENERGY_BAR_H - filled, ENERGY_BAR_W, filled);
            }
        }
    }
    ...
    protected void renderTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (isHovering(ENERGY_BAR_X, ENERGY_BAR_Y, ENERGY_BAR_W, ENERGY_BAR_H, mouseX, mouseY)) {
            ...
            guiGraphics.renderComponentTooltip(font, List.of(
                    Component.translatable("tooltip.agritechevolved.burner.energy", fmt.format(energy), fmt.format(maxEnergy)).withStyle(ChatFormatting.YELLOW),
                    Component.translatable("tooltip.agritechevolved.burner.energy_percentage", String.format("%.1f", pct)).withStyle(ChatFormatting.GRAY)
            ), mouseX, mouseY);
        }
        if (isHovering(PROGRESS_BAR_X, PROGRESS_BAR_Y - 1, PROGRESS_BAR_W, PROGRESS_BAR_H + 1, mouseX, mouseY)) {
            ...
            if (maxProgress > 0) {
                ...
                tooltip = List.of(
                        Component.translatable("tooltip.agritechevolved.burner.burning_progress").withStyle(ChatFormatting.GOLD),
                        Component.translatable("tooltip.agritechevolved.burner.progress_percentage", String.format("%.1f", pct)).withStyle(ChatFormatting.YELLOW),
                        Component.translatable("tooltip.agritechevolved.burner.time_remaining", String.format("%.1f", remainingSecs)).withStyle(ChatFormatting.GRAY)
                );
            } else {
                tooltip = List.of(
        ...
        if (isHovering(FUEL_SLOT_X, FUEL_SLOT_Y, FUEL_SLOT_W, FUEL_SLOT_H, mouseX, mouseY) && menu.blockEntity.inventory.getStackInSlot(0).isEmpty()) {
            guiGraphics.renderComponentTooltip(font, List.of(
    ...
    private boolean isHovering(int wx, int wy, int ww, int wh, int mouseX, int mouseY) {
        int x = (width - imageWidth) / 2;
        int y = (height - imageHeight) / 2;
        return mouseX >= x + wx && mouseX <= x + wx + ww && mouseY >= y + wy && mouseY <= y + wy + wh;
    }
}